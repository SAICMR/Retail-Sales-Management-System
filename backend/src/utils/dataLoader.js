...
export async function loadSalesData() {
  try {
    ...
    if (!fs.existsSync(dataPath)) {
      console.log('No data file found, generating sample data...');
...
function generateSampleData() {
  ...
  const names = ['John Doe', 'Jane Smith', 'Bob Johnson', 'Alice Williams', 'Charlie Brown', 
                 'Diana Prince', 'Ethan Hunt', 'Fiona Apple', 'George Lucas', 'Hannah Montana'];
  ...
  for (let i = 1; i <= 150; i++) {
    ...
    sampleData.push({
      ...
      storeLocation: `${regions[Math.floor(Math.random() * regions.length)]} Store`,
  ...
  fs.writeFileSync(path.join(dataDir, 'sales_data.json'), JSON.stringify(sampleData, null, 2));
